...
export default function QuestionTitles({ questions, index }) {
    ...
    return ( <
        div className = "que-details my-3"
        key = { index } >
        <
        div style = {
            { fontSize: "13px" } }
        className = "m-2 text-end pe-2" >
        <
        div className = "side-details m-1" > { Number(question.upVote.length) - Number(question.downVote.length) } { " " }
        votes <
        /div> <
        div className = "side-details m-1 text-nowrap" > { question.answer.length }
        answers <
        /div> <
        div className = "side-details m-1 text-muted" > { Number(views) }
        views < /div> <
        /div> <
        ...
        onClick = {
            () => handleViews() } > { question.questionTitle } <
        /Link> <
        ...
        style = {
            { fontSize: "13px" } } > { question.userPosted } { " " } <
